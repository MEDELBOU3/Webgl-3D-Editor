...
function addLight(type) {
 ...
 function updateMaterialEditor() {
    ...
    const properties = {
        'color': { type: 'color', value: material.color.getHexString() },
        'metalness': { type: 'range', value: material.metalness, min: 0, max: 1, step: 0.01 },
        'roughness': { type: 'range', value: material.roughness, min: 0, max: 1, step: 0.01 },
        'opacity': { type: 'range', value: material.opacity, min: 0, max: 1, step: 0.01 },
        'transparent': { type: 'checkbox', value: material.transparent },
        'wireframe': { type: 'checkbox', value: material.wireframe }
    };

    // Add advanced properties for MeshPhysicalMaterial
    if (material instanceof THREE.MeshPhysicalMaterial) {
        Object.assign(properties, {
            'clearcoat': { type: 'range', value: material.clearcoat, min: 0, max: 1, step: 0.01 },
            'clearcoatRoughness': { type: 'range', value: material.clearcoatRoughness, min: 0, max: 1, step: 0.01 },
            'transmission': { type: 'range', value: material.transmission, min: 0, max: 1, step: 0.01 },
            'thickness': { type: 'range', value: material.thickness, min: 0, max: 5, step: 0.1 },
            'ior': { type: 'range', value: material.ior, min: 1, max: 2.333, step: 0.01 }
        });
    }
    ...
    for (const [prop, config] of Object.entries(properties)) {
        ...
        container.className = 'material-control';
        ...
        input.addEventListener('input', (e) => {
            let value = config.type === 'checkbox' ? e.target.checked : 
                       config.type === 'color' ? '#' + e.target.value :
    ...
    presetContainer.className = 'material-presets';
    
    for (const preset of Object.keys(MaterialPresets)) {
        const button = document.createElement('button');
        button.textContent = preset;
        button.onclick = () => applyMaterialPreset(selectedObject, preset);
        presetContainer.appendChild(button);
    }
    
    editor.appendChild(presetContainer);
}
...
class Timeline {
 ...
 createUI() {
     ...
     <div class="timeline-controls">
         ...
         <button id="timeline-play"><i class="fas fa-play"></i></button>
         <button id="timeline-pause"><i class="fas fa-pause"></i></button>
         <button id="timeline-stop"><i class="fas fa-stop"></i></button>
         <button  class="timeline-button" id="add-track">Add Track</button>
         <button class="timeline-button" id="timeline-add-keyframe">+ Keyframe</button>
         <span id="time-display">0:00</span>
         <input type="range" id="timeline-scrubber" min="0" max="100" value="0" style="width: 60px">
 ...
 createKeyframeMarkers(track) {
    return track.keyframes.map(kf => {
        const left = (kf.time / this.duration) * 100;
        return `<div class="keyframe-marker" style="left: ${left}%"></div>`;
    }).join('');
}
 togglePlay() {
 ...
 addKeyframe(track, time, value) {
    track.keyframes.push({
        time: time,
        value: value instanceof THREE.Vector3 ? value.clone() : value
    });
...
updateObjects() {
    for (const track of this.tracks) {
        ...
        for (let i = 0; i < track.keyframes.length - 1; i++) {
            if (track.keyframes[i].time <= this.currentTime && 
                track.keyframes[i + 1].time > this.currentTime) {
        ...
        // Interpolate
        const alpha = (this.currentTime - k1.time) / (k2.time - k1.time);
        
        if (track.object[track.property] instanceof THREE.Vector3) {
            track.object[track.property].lerpVectors(k1.value, k2.value, alpha);
        } else if (track.object[track.property] instanceof THREE.Quaternion) {
            track.object[track.property].slerpQuaternions(k1.value, k2.value, alpha);
        } else {
            track.object[track.property] = THREE.MathUtils.lerp(k1.value, k2.value, alpha);
        }
    }
}
    ...
    for (const track of this.tracks) {
        ...
        trackElement.innerHTML = `
            <div class="track-header">
                ${track.object.name || 'Object'} - ${track.property}
            </div>
            <div class="track-content">
                ${this.createKeyframeMarkers(track)}
            </div>
        `;
...
function setupTimelineControls() {
    document.getElementById('timeline-play').onclick = () => timeline.play();
    document.getElementById('timeline-pause').onclick = () => timeline.pause();
    document.getElementById('timeline-stop').onclick = () => timeline.stop();
    
    document.getElementById('add-keyframe').onclick = () => {
        if (selectedObject && timeline.selectedTrack) {
            const property = timeline.selectedTrack.property;
            const value = selectedObject[property];
            timeline.addKeyframe(timeline.selectedTrack, timeline.currentTime, value);
        }
    };
}

// مثال على إضافة مسار جديد
document.getElementById('add-track').addEventListener('click', () => {
    if (selectedObject) {
        const trackType = prompt('اختر نوع المسار (position, rotation, scale):');
        if (trackType) {
            const track = timeline.addTrack(selectedObject, trackType);
            console.log('تم إضافة مسار جديد:', track);
        }
    } else {
        alert('الرجاء اختيار كائن أولاً');
    }
});

// تحديث شريط التمرير مع الوقت الحالي
function updateScrubber() {
    const scrubber = document.getElementById('timeline-scrubber');
    scrubber.value = (timeline.currentTime / timeline.duration) * 100;
    
    const minutes = Math.floor(timeline.currentTime / 60);
    const seconds = Math.floor(timeline.currentTime % 60);
    document.getElementById('time-display').textContent = 
        `${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}`;
}

// إضافة مستمع لشريط التمرير
document.getElementById('timeline-scrubber').addEventListener('input', (e) => {
    const percent = e.target.value / 100;
    timeline.currentTime = timeline.duration * percent;
    timeline.updateObjects();
});

// مثال على كيفية استخدام التايملاين
function animateObject() {
    // إنشاء مسار للموضع
    const positionTrack = timeline.addTrack(selectedObject, 'position');
    
    // إضافة keyframes
    timeline.addKeyframe(positionTrack, 0, new THREE.Vector3(0, 0, 0));
    timeline.addKeyframe(positionTrack, 2, new THREE.Vector3(5, 5, 0));
    timeline.addKeyframe(positionTrack, 4, new THREE.Vector3(5, 0, 5));
